...
const Newtask = ({data}) => {
  return (
    <div className="w-[300px] bg-blue-400 h-full flex-shrink-0 rounded-xl p-5 flex flex-col justify-between">
      <div>
        <div className='flex justify-between items-center'>
          <h3 className="text-sm bg-blue-600 px-3 py-1 rounded font-semibold text-white">{data.catagory}</h3>
          <h4 className="text-sm font-medium">{data.taskDate}</h4>
        </div>
        {/* Match the key name: taskTittle */}
        <h2 className="mt-5 text-2xl font-bold text-white">{data.taskTittle}</h2> 
        <p className="text-sm mt-2 leading-tight">
           {data.taskdescriptin}
        </p>
      </div>
      <div className='mt-5'>
        <button className='bg-blue-600 w-full py-2 font-semibold rounded text-white text-sm'>
          Accept Task
        </button>
      </div>
    </div>
